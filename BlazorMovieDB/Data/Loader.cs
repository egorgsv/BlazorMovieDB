...
namespace BlazorMovieDB.Data
{
    public class Loader
    {
        ...
        {
            List<Actor> listobj = new List<Actor>();
            using (ApplicationContext db = new ApplicationContext())
            {
                IEnumerable<Actor> whatsactors =
                from act in db.Actors
                where act.Name == name
                select act;

                listobj = whatsactors.ToList();
        ...
        {
            List<Movie> listobj = new List<Movie>();
            using (ApplicationContext db = new ApplicationContext())
            {

                IEnumerable<Movie> whatsmovies =
                from mov in db.Movies
                where mov.Title == name
                select mov;


                listobj = whatsmovies.ToList();
        ...
        {
            List<Tag> listobj = new List<Tag>();
            using (ApplicationContext db = new ApplicationContext())
            {
                IEnumerable<Tag> whatstags =
                            from tag in db.Tags
                            where tag.tag == name
                            select tag;
                listobj = whatstags.ToList();
        ...
        public void Load()
        {
            using (ApplicationContext db = new ApplicationContext())
            {
                ...
                //do
                //{
                //    Console.WriteLine("Input comand (film, actor, tag, exit):");
